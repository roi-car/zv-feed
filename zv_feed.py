...
def clean_link(url: str) -> str:
    """Fix the double/triple-encoded-ampersand bug in scraped hrefs.
    ...
    Decode fully, then re-encode '&' as a single, correct '%26' — the
    format the live site actually resolves. Every other character is left
    exactly as scraped, since things like a literal '%' in a product name
    (e.g. '100%-silk') are already correct as-is and don't need touching.
    """
    decoded = full_unquote(url)
    return decoded.replace("&", "%26")
