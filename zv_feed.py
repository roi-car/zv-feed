...
def clean_link(url: str) -> str:
    """Fix the double/triple-encoded-ampersand bug in scraped hrefs, and
    correctly re-encode any literal '%' that survives full_unquote().

    full_unquote() is already applied when parsing path components for
    metadata (sku/color/category), but the raw `url` itself was previously
    passed straight through to the feed's <link> field unchanged — so any
    over-encoding baked into the site's own hrefs (e.g. '%252526' for a
    category like "Dresses & Skirts") ended up live in the feed, producing
    a link Google/Meta can't actually fetch (400 Bad Request).

    Decode fully, then re-encode both '&' and bare '%' as their correct
    percent-encoded forms. A literal '%' is NOT safe to leave as-is: many
    product titles bake in a fabric composition like "100% Silk" or
    "100% Cashmere", and the resulting slug (e.g. 'risty-100%-silk-dress.html')
    has a '%' not followed by two hex digits — an invalid escape sequence
    that servers reject outright (400 Bad Request / GMC "Product page
    unavailable"). This previously broke every such product's link.

    Order matters: escape bare '%' FIRST, then encode '&' as '%26' —
    otherwise the '%' just inserted for '&' would get re-escaped too.
    """
    decoded = full_unquote(url)
    decoded = decoded.replace("%", "%25")   # escape literal % first
    decoded = decoded.replace("&", "%26")   # then encode &
    return decoded
...
def main():
    ...
    ap.add_argument("--no-mirror-images", action="store_true",
                    help="Keep Fastmag CDN image URLs (skip self-hosting on GitHub Pages)")
    ap.add_argument("--refresh-images", action="store_true",
                    help="Re-download all hosted images, not just missing ones")
    args = ap.parse_args()
    ...
    # not prune the hosted image folder.
    full_run = not args.limit and not args.urls
    if image_mirror.SELF_HOST_IMAGES and not args.no_mirror_images:
        image_mirror.mirror_images(
            products,
            user_agent=USER_AGENT,
            refresh=args.refresh_images,
            allow_prune=full_run,
        )
